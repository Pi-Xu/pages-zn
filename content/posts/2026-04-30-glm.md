...
在用 $\mu=b'(\theta)$ 替换 $\theta$ 后，叫作 variance function。
...
$$
b'(\theta)=\frac{e^\theta}{1+e^\theta}=\mu,
$$
...
- 条件均值是 $\mu_i=E(Y_i\mid x_i)$，指数族给出 $\mu_i=b'(\theta_i)$。
...
| 响应分布      |        $b(\theta)$ |              $\mu=b'(\theta)$ | canonical link                 |
| --------- | -----------------: | ----------------------------: | ------------------------------ |
...
第三个 insight 是方差结构。GLM 不再默认所有响应变量都有常方差。Bernoulli、Poisson、Gamma 等分布都有自己的 mean-variance relationship，而这个关系正是通过 $b''(\theta)$ 或 $V(\mu)$ 体现出来的。
...
$$
\mu_i=E(Y_i\mid x_i)=b'(\theta_i)
$$

$$
\operatorname{Var}(Y_i\mid x_i)=\phi b''(\theta_i)=\phi V(\mu_i)
$$
